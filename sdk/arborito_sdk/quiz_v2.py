...
def load_arborito_archive(path: Any) -> dict[str, Any]:
    ...
    ``lessons/<LANG>/<NN module>/<NN leaf>.md``. Folder / file order comes
    from the leading ``NN -`` numeric prefix and the display name is whatever
    follows it. When the author wants characters that the filesystem cannot
    keep in the filename (`/`, `\\`, `:`, `*`, `?`, `"`, `<`, `>`, `|`),
    they add a ``title:`` line inside the ``@info`` block — that title takes
    precedence over the filename. Per-node properties (icon, description,
    exam, review, …) live in the same ``@info … @/info`` block. The returned
    dict has the standard shape ``{ magic, version, meta, tree, files? }``
...
def _collect_children(
    *,
    ...
    for rel, _ in lang_entries.items():
        ...
        slash = tail.find("/")
        if slash < 0:
            if tail.startswith("_") or not tail.lower().endswith(".md"):
                continue
...
def _build_branch(
    *,
    ...
    branch_meta_bytes = lang_entries.get(f"{relative_prefix}_branch.md")
    branch_meta = _parse_leaf_header(branch_meta_bytes.decode("utf-8")) if branch_meta_bytes else {}
    name = branch_meta.get("title") or fallback_name
    ...
    return {
        ...
        "description": branch_meta.get("description") or "",
        "expanded": True,
        "children": children,
    }
