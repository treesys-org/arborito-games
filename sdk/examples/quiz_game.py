...
- Loads any .arborito archive (pass `--arborito` or place `ingles-a1-demo.arborito` next to the script).
...
Usage:
    ...
    python3 examples/quiz_game.py --arborito ./salida.arborito --rounds 20
...
def _default_tree_path() -> Path:
    ...
    for candidate in (
        here / "sample.arborito",
        here / "ingles-a1-demo.arborito",
        Path.cwd() / "ingles-a1-demo.arborito",
    ):
        if candidate.is_file():
            return candidate
    return here / "sample.arborito"


DEFAULT_TREE = _default_tree_path()
