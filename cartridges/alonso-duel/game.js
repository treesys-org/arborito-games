...
Object.assign(AlonsoDuel.prototype, {
    ...
    waitPlayerAnswer(card) {
        return new Promise((resolve) => {
            ...
            this.renderBattleCard(card, { showOpts: true, optsDisabled: false, owner: 'alonso' });
            ...
            if (isSequenceMode(card)) {
                const picked = [];
                const pickedEl = slot.querySelector('.seq-picked-val');
                const chips = [...slot.querySelectorAll('.seq-chip')];
                const submitBtn = slot.querySelector('.seq-submit');
                const expectedLen = Array.isArray(card.sequence) ? card.sequence.length : chips.length;

                const refreshPickedLabel = () => {
                    if (pickedEl) pickedEl.textContent = picked.join(' → ');
                };
                const submitNow = () => {
                    if (this.locked || done) return;
                    this.locked = true;
                    finish(checkSequenceAnswer(card, picked));
                };

                chips.forEach((chip) => {
                    chip.onclick = () => {
                        if (this.locked || done) return;
                        /* Allow undoing the last (or any prior) pick by tapping the
                         * already-picked chip: previously chips became permanently
                         * disabled, so a single mis-tap forced you to wait for the
                         * timer. Now clicks toggle the chip's state. */
                        if (chip.classList.contains('is-picked')) {
                            const i = picked.indexOf(chip.dataset.chip);
                        ...
                        refreshPickedLabel();
                        /* Auto-submit once the order is complete so a single
                         * forgotten "Confirmar" tap doesn't time out a perfect
                         * answer — players in fast rounds were losing rounds
                         * even after laying down the correct sequence. */
                        if (expectedLen > 0 && picked.length >= expectedLen) {
                            submitNow();
                        }
                    };
                });
    ...
    startBlockTimer(onTimeout) {
        ...
        let left = BLOCK_TIME_SEC;
        const circumference = 94.25;
        const updateRing = () => {
            const pct = left / BLOCK_TIME_SEC;
    ...
    async animateAlonsoAnswer(card) {
        ...
        if (isSequenceMode(card)) {
            ...
            /* Show the answer above his portrait so it's actually readable
               while the chips animate. The small narration line below
               receives the result message after the reveal. */
            this.showAlonsoSpeech(answerLine, { holdMs: PAUSE_REVEAL + PAUSE_RESULT + 400 });
            let delay = 0;
            pickOrder.forEach((word) => {
                const chip = chips.find((c) => c.dataset.chip === word);
                if (chip) {
                    setTimeout(() => chip.classList.add('alonso-pick'), delay);
                    delay += 260;
                }
            });
            await wait(PAUSE_REVEAL);
            chips.forEach((c) => c.classList.remove('alonso-pick'));
