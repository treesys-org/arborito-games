...
class Game {
 ...
 /** Contextual guidance, never dump every key. Soft movement tip once,
 * then the world (LAND near planet, TALK near NPC) teaches the rest. */
 ...
 const title = toast?.querySelector('.control-toast-title');
 ...
 clearTimeout(this._toastTimer);
 this._toastTimer = setTimeout(dismiss, 5200);
 window.addEventListener('keydown', dismiss, { once: true });
 window.addEventListener('pointerdown', dismiss, { once: true });
 }

 /** Floating event feedback (+ammo, crystal, objective…). */
 showFx(message, kind = 'quest', ms = 1800) {
