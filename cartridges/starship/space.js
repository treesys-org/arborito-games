...
export class SpaceEngine {
    ...
    initTouch() {
        ...
        this.btnFire?.addEventListener('touchstart', (e) => { e.preventDefault(); this.input.setKey('z', true); });
        this.btnFire?.addEventListener('touchend', (e) => { e.preventDefault(); this.input.setKey('z', false); });
    ...
    generateGalaxy(lessons) {
        /* Curriculum lessons are grouped in fours; each group becomes one explorable star system. */
        this.systems = [];
        this.asteroids = [];
        this.enemies = [];
        this.comets = [];
    ...
    updateProjectiles() {
        /* SPACE is reserved for "land on nearest planet" in space mode, so we
           only honor Z / J / F for ship cannons here — otherwise the player
           can't fire when there's no planet within range. */
        if (this.input.consume('z') || this.input.consume('Z') ||
