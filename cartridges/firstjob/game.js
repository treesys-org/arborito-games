...
import { isStaticMode, loadQuizPool, buildOptions, pickRandom } from './quiz-pool.js';
...
class Game {
    ...
    prepareInterviewUI() {
        ...
        if (this.staticMode) {
            ...
            this.showChoices(buildOptions(q), 'interview', q.q, q.topic);
        } else {
            this.hideChoices();
    ...
    triggerPhoneCall() {
        ...
        if (this.staticMode && this.quizPool.length > 0) {
            const preview = pickRandom(this.quizPool);
    ...
    async startMinigame(npc) {
        ...
        if (this.staticMode) {
            const quizItem = this.quizPool.length > 0 ? pickRandom(this.quizPool) : null;
            if (quizItem) {
                q = {
                    role: npc.role,
            ...
            try {
                q = await window.arborito.ask.json(`Context: "${this.contextText.substring(0,400)}". Create complaint for ${npc.role} in ${langName}. JSON: {"role":"", "complaint":""}`);
            } catch(e) {}
        }
        ...
        if (this.staticMode && q.correct) {
            this.els.taskOverlay.style.display = 'none';
            this.showChoices(buildOptions(q), 'task', q.complaint || q.q || '', q.topic || '');
