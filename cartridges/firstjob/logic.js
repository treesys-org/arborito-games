...
import { answersMatch, reportMemory, grantXp, buildOptions, resolveLessonById, gradeQuizAnswer } from './quiz-pool.js';
...
export class GameLogic {
 ...
 try {
 const lesson = await resolveLessonById(td?.lessonId);
 if (lesson && window.arborito?.ask?.lessonAction) {
 const res = await window.arborito.ask.lessonAction(lesson, val, {
 persona: `You are a ${td.role}. Problem: "${td.complaint}". IT support replied. Evaluate if the problem is solved. Reply in ${langName}, max 8 words in output.`,
 askOptions: { timeoutMs: 90000, maxAttempts: 2 },
 });
 this.finalizeTask(!!res?.success, res?.output || this.game.getLine('TICKET_WRONG'));
 return;
 }
 const ok = await gradeQuizAnswer(lesson, td, val);
 const reply = ok
 ? this.game.getLine('TICKET_CORRECT')
 : this.game.getLine('TICKET_WRONG');
 this.finalizeTask(ok, reply);
 ...
 buildOptions(td, {
 distractorPool: (this.game.quizPool || [])
 .map((item) => String(item.correct || '').trim())
 .filter(Boolean),
 lang: this.game.lang
 }),
 'task',
 ...
 try {
 const lesson = await resolveLessonById(q.lessonId) || await window.arborito.lesson.at(0);
 const approved = await gradeQuizAnswer(lesson, q, val);
 if (approved) {
 this.game.interviewScore++;
 this.game.audio.sfxSuccess();
 this.game.lastInterviewFeedback = this.game.getLine('CORRECT_FEEDBACK');
 } else {
 this.game.audio.sfxError();
 this.game.lastInterviewFeedback = this.game.getLine('WRONG_FEEDBACK');
 }
 if (q.lessonId) reportMemory(q.lessonId, approved ? 4 : 2);
 grantXp(approved ? 10 : 2);
 } catch(e) {
